...
\title{Clustering}
\subtitle{Beginner's Workshop Machine Learning 2019}
\author{M. Esders, C. Anders}
...
\begin{frame}
  ...
  \begin{center}
    \url{gitlab.tubit.tu-berlin.de/christopher.j.anders/clustering_presentation.git}\\
  \end{center}
  Contains all code to generate plots\\
  Parts not otherwise marked: BSD licensed\\
  Compiled PDF available in the project wiki!\\[3mm]
  Forked from:
  \begin{center}
    \url{gitlab.tubit.tu-berlin.de/maltimore/clustering_presentation.git}\\
  \end{center}
 ...
 \frametitle{Unsupervised Learning}
  \begin{center}
    \begin{quote}
      ...
      itself. (Geoffrey Hinton, 1996)
    \end{quote}
  \end{center}
  Genome: $3 * 10^9$ base-pairs, i.e. $6 * 10^9$ bits.
\end{frame}
...
\begin{frame}[t]
  ...
  \begin{columns}[T]
    \column{0.6\textwidth}
      ...
      \end{equation*}
      Usage: LASSO regularization (tomorrow!)
    \column{0.3\textwidth}
...
\begin{frame}[t]
  ...
  \begin{columns}[T]
    ...
    \only<4>{\
      \includegraphics[width=\textwidth]{star_scatter.png}\
      \tiny \cite{hinton_reducing_2006}\
    }\
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    ...
    \column{0.4\textwidth}
      \onslide<3->
      \includegraphics[width=\textwidth]{DBSCAN_illustration.png}\\
